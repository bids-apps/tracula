...
#### FreeSurfer
RUN apt-get update && \
  apt-get install -y tcsh bc tar libgomp1 perl-modules
...
RUN curl -sSLO https://repo.continuum.io/miniconda/Miniconda3-4.7.10-Linux-x86_64.sh && \
    bash Miniconda3-4.7.10-Linux-x86_64.sh -b -p /usr/local/miniconda && \
    rm Miniconda3-4.7.10-Linux-x86_64.sh
ENV PATH="/usr/local/miniconda/bin:$PATH"

RUN conda install -y \
    pandas joblib
RUN pip install  pybids==0.3 grabbit==0.0.8 nibabel
